...
def all_udl(fps_limiter: FpsLimiter, args: Tuple[sl.Camera], benchmarking=True):
    ...
    for acquisition_time in fps_limiter.rate(ZED_IMAGE_FPS):
        ...
        left_image = np.ascontiguousarray(to_rgb(mats["left"].get_data()))
        right_image = np.ascontiguousarray(to_rgb(mats["right"].get_data()))
        ...
        depth_ocv = np.ascontiguousarray(depth_ocv)
        normal_map += 1
        normal_map /= 2.0
        ...
        yield ZED_AGGREGATE_DIRECTION, acquisition_time, frames, names
